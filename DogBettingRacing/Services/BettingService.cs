...
namespace DogBettingRacing.Services
{
    public class BettingService : BackgroundService
    {
        ...
        public BettingService(ILogger<BettingService> logger, IServiceScopeFactory scopeFactory)
        {
            _logger = logger;
        ...
        public async Task<bool> PlaceBetAsync(AppDbContext _context)
        {
            try
            {
                ...
                var dogs = await _context.Dogs.Where(d => d.Round.Status == "Open").ToListAsync();
        ...
        {
            Console.WriteLine("Betting service executeAsync");
